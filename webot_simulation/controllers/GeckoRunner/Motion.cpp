...
# define LF 300
# define LR 400
	float Motion::getNextStep()
		return a[4][3];
		
	void Motion::TriangleGait()
	{
		for(t=0;t<step+1;t++)
		{Detach(RF);moving(RR);moving(LF);moving(LR);}
		setallcurrentposition();PrintCoordinate();
		Adhesive(RF);setallcurrentposition();PrintCoordinate();
		
		for(t=0;t<step+1;t++)
		{Detach(RR); moving(RF); moving(LF); moving(LR);} 
		setallcurrentposition();PrintCoordinate();
		Adhesive(RR);setallcurrentposition();PrintCoordinate();
		
		for(t=0;t<step+1;t++)
		{Detach(LF); moving(RF); moving(RR); moving(LR);}
		setallcurrentposition();PrintCoordinate();
		Adhesive(LF);setallcurrentposition();PrintCoordinate();

		for(t=0;t<step+1;t++)
		{Detach(LR); moving(RF); moving(RR); moving(LF);}
		setallcurrentposition();PrintCoordinate();
...
*/
	...
	void Motion::setcurrentposition(int fn)
	{
		if (fn==RF)
		{
	...
	void Motion::moving(int fn)
	{
		...
		int k=10;
		     ...
		     if(fn==RF){k=0;}
				...
				for(int n=0;n<3;n++){ a[k][n]=Decimal(a[k][n]);}
			Output_PWM(fn);
	}
